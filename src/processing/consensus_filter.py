...
from schemas.variant_trace_v1 import ACMGClassification, VariantTrace
...
class ConsensusFilter:
    ...
    def run(self):
        print("Starting CQF Filter (ZDS-ID: TOOL-702)...")
        self.output_path.parent.mkdir(parents=True, exist_ok=True)

        with open(self.input_path) as f_in, open(self.output_path, "w") as f_out:
            for line in f_in:
                if not line.strip():
                    continue
