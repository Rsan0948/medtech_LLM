...
class R1Teacher:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    raise Exception(f"Request timed out after {MAX_RETRIES} attempts ({elapsed:.1f}s total)")
    ...
    def _process_single_item(self, item: Dict, processed_ids: Set[str]) -> Dict[str, Any]:
    ...
    def process_batch(self, batch: List[Dict], processed_ids: Set[str], f_out) -> List[Dict]:
            ...
            while pending and (time.time() - batch_start) < BATCH_TIMEOUT:
                ...
                if not done:
                    ...
                    if int(elapsed) % 30 == 0 and int(elapsed) > 0:
                        print(f"  ⏳ Batch in progress... {completed_count}/{len(batch)} done, {len(pending)} pending ({elapsed:.0f}s elapsed)")
        ...
        if len(results) < len(batch):
            print(f"  ⚠️  Batch incomplete: {len(results)}/{len(batch)} succeeded ({len(batch) - len(results)} failed/timed out)")
    ...
    def run(self):
        ...
        with open(self.prompts_path, 'r') as f_in:
            all_prompts = [json.loads(l) for l in f_in if l.strip()]
        ...
        print(f"\n{'='*60}")
        print(f"R1 TEACHER - SIMPLE MODE (NO BATCHES)")
        print(f"{'='*60}")
                ...
                for future in as_completed(future_to_item):
                    try:
                        ...
                        if record:
                            ...
                            # Progress every 10
                            if completed % 10 == 0:
                                print(f"  Progress: {completed}/{len(prompts_to_process)} ({100*completed/len(prompts_to_process):.1f}%)")
                        else:
                            failed += 1
                    except Exception as e:
                        failed += 1
